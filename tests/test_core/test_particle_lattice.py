...
def test_initialize_lattice():
    ...
    assert actual_density == pytest.approx(density, abs=0.1), \
...
def test_move_particle():
    ...
    assert lattice.move_particle(3, 3, 3, 3) == False  # Attempt to move to the same spot
...
def test_reorient_particle():
    ...
    assert lattice.lattice[0, 2, 2] == False  # Check if the particle no longer has the old orientation
    assert lattice.reorient_particle(2, 2, 1) == False # Attempt to reorient to the same orientation
    assert lattice.reorient_particle(2, 2, 4) == False # Attempt to reorient to an invalid orientation (out of bounds)
    assert lattice.reorient_particle(3, 3, 2) == False # Attempt to reorient a non-existent particle
...
def test_get_statistics():
    ...
    assert 0 <= stats['order_parameter'] <= 1  # Order parameter should be between 0 and 1
