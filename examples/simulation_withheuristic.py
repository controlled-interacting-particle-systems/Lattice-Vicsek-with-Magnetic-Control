...
def main():
    ...
    g = float(sys.argv[3])  # Alignment sensitivity
    v0 = float(sys.argv[4])  # Base transition rate
    v1 = float(sys.argv[5]) # flow rate 
    ...
    t_last = 0
    n_steps = int(2.e6) # Number of steps to run the simulation for
    ...
    #loop seed#
        fileObject = open(f"data/order_params_size{height}{width}_seed{seed}_g{g}_v0{v0}_v1{v1}.dat", "w")
        fileDensity_y = open(f"data/density_y_size{height}{width}_seed{seed}_g{g}_v0{v0}_v1{v1}.dat", "w")
        fileVely_x = open(f"data/meanvely_x_size{height}{width}_seed{seed}_g{g}_v0{v0}_v1{v1}.dat", "w")
        fileVely_y = open(f"data/meanvely_y_size{height}{width}_seed{seed}_g{g}_v0{v0}_v1{v1}.dat", "w")
        file_particles_in_sinks = open(f"data/numparticles_in_sinks{height}{width}_g{g}_seed{seed}_v0{v0}_v1{v1}.dat", "w")
        file_lattice = open(f"data/lattice_saved_{height}{width}_seed{seed}_g{g}_v0{v0}_v1{v1}.dat", "w")
        ...
        #fileVely_x.close()
        #fileVely_y.close()
        #file_particles_in_sinks.close()
        #file_lattice.close()
    #loop seed# #end loop seed 
