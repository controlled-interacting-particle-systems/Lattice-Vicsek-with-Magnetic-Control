...
the model of [@peruani2011traffic]. Each lattice site can only be
occupied by a single particle. The particle labeled $k\in[1..N_{\rm p}]$
has an orientation $\boldsymbol{p}_{k}$, which can take one of the four
discrete values: up $\uparrow\ = (0,1)$, down $\downarrow\ = (0,-1)$,
left $\leftarrow\ = (1,0)$, or right $\rightarrow\ = (-1,0)$. Particles
move to the next grid point pointed by their direction with a rate
$v_0$, only when this site is not occupied by another particle.
Additionally, they change their orientation to $\boldsymbol{p}'$ with a
rate
...
with a rate $\mu$. These settings align with those studied in
[@peruani2011traffic], focusing on square periodic domains.
...
Fig. [1](#fig:sketch_lattice_channel){reference-type="ref"
reference="fig:sketch_lattice_channel"}, left). We use the same
parameter as for interactions between particles, as both cases involve
reorientation due to hydrodynamical interactions. The factor $1/2$ aims
to equally distribute possible orientations between the two
perpendicular directions. [Is this really justifying the $1/2$ inside
the exponential?]{style="color: red"}

![[]{#fig:sketch_lattice_channel
label="fig:sketch_lattice_channel"}Left: illustration of the
reorientation induced by an obstacle (red dot) on a particle (blue dot)
that faces it. Right: Periodic channel of size $N_x\times N_y$ with two
walls of obstacles (top and bottom rows of red
dots).](sketch_lattice_channel.pdf){#fig:sketch_lattice_channel
width=".8\\columnwidth"}
...
Fig. [1](#fig:sketch_lattice_channel){reference-type="ref"
reference="fig:sketch_lattice_channel"}, right). We maintain periodic
boundary conditions in the $x$ direction. The dynamics of interacting
active particles and swarms in such a configuration have been studied in
the continuous case [@armbruster2017swarming] and for a hybrid lattice
method [@kuhn2021lattice]. Note that these studies considered reflective
boundary conditions, which slightly differ from our preferential
alignment.
...
or lower half of the channel. [The factor 2 is in the code but we should
check if this makes sense.]{style="color: red"}
...
reference="tab:parameters"} summarizes the parameters of our model that
remain after this choice, with some comments on those which we choose to
fix and those to vary.
...
Tab. [1](#tab:parameters){reference-type="ref"
reference="tab:parameters"} to explore the parameter plane $(g,v_1)$. To
characterize different phases, we measure the classical order parameter
given by the instantaneous mean particle orientation:
...
Fig. [2](#fig:compare_times){reference-type="ref"
reference="fig:compare_times"}. It seems there that physical time runs
slightly slower during transients, both at beginning of the simulation
and around $t=100$ when a transition occurs. This comes from the fact
that in the steady configuration, which consists of bands, global
rotation and migration rates are longer than in a situation where more
particles can rotate or move. In the sequel, we are considering averages
over events only, but this point should be kept in mind. [There is a
point which is still unclear to me on how to define time, related to the
fact that we removed the "null events" (when nothing changes in the
...
class="math inline"><em>Π</em></span> for <span
class="math inline"><em>g</em> = 1.75</span> for <span
class="math inline"><em>v</em><sub>1</sub> = 15</span> as a function of
iteration (up) and of physical time (down).</figcaption>
</figure>
...
::: figure*
![image](ord_param_v1_15_fng){width=".32\\textwidth"}
![image](mean_density_v1_15_fny){width=".32\\textwidth"}
![image](ord_param_phi_v1_15_fny){width=".32\\textwidth"}

![image](snap_g0.25_v1_15_t2e6){width=".25\\textwidth"}
![image](snap_g0.75_v1_15_t2e6){width=".25\\textwidth"}
![image](snap_g1.5_v1_15_t2e6){width=".25\\textwidth"}
![image](snap_g3_v1_15_t2e6){width=".25\\textwidth"}
:::
...
Fig. [\[fig:ordparam_noflow\]](#fig:ordparam_noflow){reference-type="ref"
reference="fig:ordparam_noflow"}. [(I didn't have the data for $v_1=0$,
but I guess this should not change the qualitative gross
behaviors.)]{style="color: red"} As anticipated, the left panel
illustrates that the order increases with alignment. However, at small
values of $g$, particles tend to form clusters at the boundaries.
Consequently, $\Phi$ exhibits larger values at small $g$'s than expected
in the case of periodic boundary conditions. It is also noteworthy that
the clustering order parameter, $\Psi$, is not particularly effective in
distinguishing between the various possible phases, as it assumes
relatively large values across all $g$ values. Nevertheless, we retain
it in the analysis, considering that it might provide important
information when implementing policies.
...
Fig. [\[fig:snapshots\]](#fig:snapshots){reference-type="ref"
reference="fig:snapshots"}. One observes, at small $g$'s, the formation
of mountain-like clusters at the boundaries. At large values of $g$, the
particles rather form bands that can occupy both boundaries or only one
of the two.
...
class="math inline"><em>Φ</em></span>, <span
class="math inline"><em>Φ</em><sub><em>x</em></sub></span> and <span
class="math inline"><em>Φ</em><sub><em>y</em></sub></span> for <span
class="math inline"><em>v</em><sub>1</sub> = 15</span> and <span
class="math inline"><em>g</em> = 2</span>. Bottom, left: <span
class="math inline"><em>y</em></span>-dependent order parameter <span
class="math inline"><em>Φ</em><sub><em>j</em></sub></span> averaged in
time from iteration <span class="math inline">4 × 10<sup>5</sup></span>.
Right panels: Configurations of the lattice at the three different times
<span
class="math inline"><em>t</em><sub>1</sub> &lt; <em>t</em><sub>2</sub> &lt; <em>t</em><sub>3</sub></span>
(from top to down), instants shown as vertical dashed lines in the
top-left panel.</figcaption>
</figure>

::: figure*
![image](ord_param_var_v1_fng){width=".32\\textwidth"}
![image](mean_dist_var_v1_fng){width=".32\\textwidth"}
![image](ord_param_fnv1_and_g){width=".32\\textwidth"}

![image](snap_g0.5_v1_30_t2e6){width=".25\\textwidth"}
![image](snap_g0.5_v1_125_t2e6){width=".25\\textwidth"}
![image](snap_g2.5_v1_30_t2e6){width=".25\\textwidth"}
![image](snap_g2.5_v1_125_t2e6){width=".25\\textwidth"}
:::
...
Fig. [3](#fig:g2_v1_15){reference-type="ref" reference="fig:g2_v1_15"}
(right panels) for $g=2$. After remaining in a band configuration along
the top wall for more than $10^6$ steps, all particles abruptly migrate
to reach the lower wall. During the transition, they form a structure
close to a "traffic jam" along the bottom wall. The evolution of the
order parameter and its components for this transition is shown in the
top-left panel of Fig. [3](#fig:g2_v1_15){reference-type="ref"
reference="fig:g2_v1_15"}. It is also evident that the typical
...
Figs. [\[fig:param_space\]](#fig:param_space){reference-type="ref"
reference="fig:param_space"} and
[\[fig:snapshots_phases\]](#fig:snapshots_phases){reference-type="ref"
reference="fig:snapshots_phases"}. There are two main observations.
First, at small $g$, the effect of a mean flow is to decrease clustering
at the wall and thus to impede the formation of "mountains" (compare
cases A and B). Second, at large values of $g$, the presence of a mean
flow make the particle form bands at the center of the channel rather
than near its walls (compare cases C and D).
...
\nonumber$$ [Measurements of this flux are not yet implemented in the
code. One should add a counter that is updated at each horizontal
migration.]{style="color: red"}
...
Fig. [4](#fig:sketch_heuristics){reference-type="ref"
reference="fig:sketch_heuristics"}).

![[]{#fig:sketch_heuristics label="fig:sketch_heuristics"} Sketch of the
heuristic policy.](sketch_heuristics){#fig:sketch_heuristics
width="80%"}

Preliminary results with this control show that it is able to disrupt
the clusters, at leat at small values of $g$ (see
Fig. [5](#fig:swarm_channel){reference-type="ref"
reference="fig:swarm_channel"}). [Further analysis, and in particular a
quantification of fluxes, is required.]{style="color: red"}
...
class="math inline"><em>g</em> = 1</span> and <span
class="math inline"><em>v</em><sub>1</sub> = 50</span>.</figcaption>
</figure>

# Todos

[**In the code of the model**]{style="color: red"}\
[$\bullet$ Implement the restarts]{style="color: red"}\
[$\bullet$ Add measurement of the fluxes]{style="color: red"}\
[$\bullet$ Recompute the rates only locally and not globally at each
steps]{style="color: red"}\
[$\bullet$ Write lattice output in a more frugal (binary)
manner]{style="color: red"}

 

[**Simulations**]{style="color: red"}\
[$\bullet$ Longer runs for more statistics with the current
resolution]{style="color: red"}\
[$\bullet$ Higher resolutions for fixed parameters $g$ and
$v_1$]{style="color: red"}\
[$\bullet$ Vary the aspect ratio (box length), keeping resolution
fixed]{style="color: red"}

 

[**Learning**]{style="color: red"}\
[$\bullet$ Implement the gymnasium wrappers]{style="color: red"}\
[$\bullet$ Test usual $Q$ learning with simple discretised
states]{style="color: red"}\
[$\bullet$ Test deep $Q$-learning with the whole lattice as an input of
the network]{style="color: red"}
